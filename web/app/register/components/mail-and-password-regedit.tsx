...
export default function MailAndPasswordAuth({ isInvite, isEmailSetup, allowRegistration }: MailAndPasswordAuthProps) {
  ...
  const handleEmailPasswordregister = async () => {
    if (!name) {
      Toast.notify({ type: 'error', message: t('login.error.nameEmpty') })
      return
    }
    if (!email) {
    ...
    if (password != password2) {
      Toast.notify({ type: 'error', message: t('login.error.password2') })
      return
    }
    try {
      setIsLoading(true)
      const registerData: Record<string, any> = {
        ...
        password2,
        language: locale,
        tenant_names: decodeURIComponent(searchParams.get('tenant_names') as string),
      }
  ...
  return <form onSubmit={() => { }}>
    ...
    <div className='mb-3'>
      <label htmlFor="password2" className="my-2 flex items-center justify-between">
        <span className='system-md-semibold text-text-secondary'>{t('login.password2')}</span>
      </label>
      <div className="relative mt-1">
        <Input
          id="password2"
          ...
          type={showPassword ? 'text' : 'password'}
          autoComplete="current-password"
          placeholder={t('login.password2Placeholder') || ''}
          tabIndex={3}
        />
        <div className="absolute inset-y-0 right-0 flex items-center">
          <Button
