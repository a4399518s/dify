...
import DeleteAccount from '../delete-my'
...
import { getAiInfo } from '@/service/msh'
...
export default function AccountPage() {
  ...
  useEffect(() => {
    const loadAiInfo = async () => {
      const aiInfo = await getAiInfo('/account/ai-info');
      console.log(aiInfo)
      setAiInfo(aiInfo);
    };
    loadAiInfo();
  ...
  const copy = () => {
    ...
    textarea.value = userProfile.email;
    ...
    try {
      ...
      notify({ type: 'success', message: success?"复制成功":"复制失败" })
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className='mb-8 flex items-center rounded-xl bg-gradient-to-r from-background-gradient-bg-fill-chat-bg-2 to-background-gradient-bg-fill-chat-bg-1 p-6'>
        ...
        <div style={{overflow: 'hidden'}} className='ml-4'>
          <p className='system-xl-semibold text-text-primary'>{userProfile.name}</p>
          <p style={{ whiteSpace: 'nowrap' }} className='system-xs-regular text-text-tertiary'>{userProfile.email}</p>
        </div>
      </div>
      <div className='mb-8'>
      ...
      <div className='mb-8'>
        ...
        <div  style={{ whiteSpace: 'nowrap' }} className='mt-2 flex w-full items-center justify-between gap-2'>
          <div  style={{ overflow: 'hidden' }} className='system-sm-regular flex-1 rounded-lg bg-components-input-bg-normal p-2 text-components-input-text-filled '>
            <span  className='pl-1'>{userProfile.email}</span>
          </div>
          <div className='system-sm-medium cursor-pointer rounded-lg bg-components-button-tertiary-bg px-3 py-2 text-components-button-tertiary-text' onClick={copy}>
            复制
          </div>
        </div>
      </div>
      ...
      <div className='mb-8'>
        ...
        <div className={titleClassName} style={{ marginTop: "10px"}}>数字人形象</div>
        {aiInfo?.szrxx === undefined || aiInfo?.szrxx === null ? (
          <div className={descriptionClassName}>您的数字人形象尚未生成</div>
        ) : aiInfo.szrxx.status == "等待" || aiInfo.szrxx.status == "生成中" ? (
          <div className={descriptionClassName}>您的数字人形象正在生成</div>
        ):(
          <div className={descriptionClassName}>您的数字人形象已生成</div>
        )}
        <div className={titleClassName} style={{ marginTop: "10px"}}>声纹</div>
        {aiInfo?.szrsw === undefined || aiInfo?.szrsw === null ? (
          <div className={descriptionClassName}>您的数字人声纹尚未生成</div>
        ) : aiInfo.szrsw.status == "等待" || aiInfo.szrsw.status == "生成中" ? (
          <div className={descriptionClassName}>您的数字人声纹正在生成</div>
        ):(
          <div className={descriptionClassName}>您的数字人声纹已生成</div>
        )}
        <div className={titleClassName} style={{ marginTop: "10px"}}>您的余额</div>
        <div className={descriptionClassName}>{userProfile.point}</div>
        {!!apps.length && (
          <Collapse
